			...
			<form method="post" action="">
				...
				<article>
					<h3>Localisation</h3>
						...
						<?php
							if(isset($_POST)) {
								$localisation['Pays']= $db->quote($_POST["pays"]);
								$localisation['Region']=$db->quote($_POST["region"]);
								$localisation['Ville']=$db->quote($_POST["ville"]);
								...
								$contraintes['animaux_autorises']=$db->quote(isset($_POST["animaux_autorises"]) ? $_POST["animaux_autorises"] : '');
								$contraintes["equipement"]=$db->quote(isset($_POST["equipement"]) ? $_POST["equipement"] : '');
								$contraintes["acces_handicape"]=$db->quote(isset($_POST["acces_handicape"]) ? $_POST["acces_handicape"] : '');
								$contraintes["fumeur"]=$db->quote(isset($_POST["fumeur"]) ? $_POST["fumeur"] : '');
								$contraintes["equipements_sportifs"]=$db->quote(isset($_POST["equipements_sportifs"]) ? $_POST["equipements_sportifs"]);
								...
								$sql = 'INSERT INTO recherche (localisation_pays, localisation_region, localisation_ville,caracteristiques_sejour, caracteristiques_pieces, caracteristiques_personnes, parking, piscine, climatisation, wifi, animaux_autorises, equipement, acces_handicape, fumeur, equipements_sportifs,commerces, debut, fin, remarques) VALUES (' . $localisation['Pays'] . ', ' . $localisation['Region'] . ', ' . $localisation['Ville'] .','. $caracteristiques['sejour'] .','   . $caracteristiques['pieces'] .',' . $caracteristiques['personnes'] .',' . $amenagement['parking'] .',' .$amenagement['piscine'] .','.$     ')';
								// print_r($sql);
								$req = $db->query($sql);
						...
						<div class="contraintes">
							<input type="radio" name="animaux autorisés" id="animaux_autorises"/> <label for="animaux autorisés">Animaux autorisés</label><br/>
						</div>
						<div class="contraintes">
							<input type="radio" name="equipement" id="equipement"/> <label for="equipement">Equipements adaptés aux enfants</label><br/>
						</div>
						<div class="contraintes">
							<input type="radio" name="accès handicapé" id="acces_handicape"/> <label for="accès handicapé">Accès handicapé</label><br/>
						</div>
						...
						<div class="contraintes">
							<input type="radio" name="équipements sportifs" id="equipements_sportifs"/> <label for="équipements sportifs">Equipements sportifs</label><br/>
						</div>
						<div class="contraintes">
